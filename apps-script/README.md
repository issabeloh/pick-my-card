...
| 封存已處理列：2-變動通知 → 2-封存 | `archiveInboxDoneRows` | `2-變動通知`（已發布／打 `X`／逾期非實質變動的列） | `2-封存（變動通知）`（沒有會自動建），整列搬過去後從主分頁刪掉 | `watchlist-monitor.gs` |
...
那是**只增不減的 log 分頁**——直接刪列會把紀錄弄丟，但一直長下去每次進來都要捲很久。
所以用搬的：選單 **「🤖 權益自動化 → 封存已處理列：2-變動通知 → 2-封存」**（`archiveInboxDoneRows`），
整列（含新舊全文）搬到 `2-封存（變動通知）`，主分頁只留還沒處理完的列。

哪些列算「處理完了」（三選一，其餘一律留著）：

| 條件 | 意思 |
|---|---|
| `公開` ＝ `已發布` | 發布過了，這列任務結束 |
| `公開` ＝ `X`／`不公開`／`略過`／`-` | **你看過、決定不公開**——就是用這個標記法：看完打個 `X`，下次封存就收走 |
| `公開` 空白 ＋ `實質變動` ＝ `否` ＋ 超過 30 天 | 純雜訊，本來就不會處理（天數 `MONITOR_CONFIG.archiveJunkAfterDays`） |

- `公開` 空白又還在 30 天內、或打了 `V` 還沒發布 → 留在主分頁（程式不猜你的意思）
- `X` 跟發布用的 `V` 不會打架：發布只認 `V`／`✓`（`isPublishMark_`），封存只認 `X` 那組（`isNoPublishMark_`）
- 搬完會照「日期時間」重排。一次最多搬 120 個連續區塊（避開 Apps Script 6 分鐘上限），還有剩會叫你再按一次
- **封存分頁純粹給人考古**，程式完全不讀它——確定用不到了整張刪掉也不會壞
- 比對基準存在 `1-監控清單` 的 `last_snapshot`，跟這張表無關，搬走不影響監控
