...
| 發布變動紀錄：2-變動通知 → 資料檔 | `publishChangelog` | `2-變動通知`（「公開」欄＝`V` 的列） | **跨檔**寫資料檔的 `變動紀錄`；回頭把「公開」改 `已發布` | `benefits-parser.gs` |
| 解析新戶活動：2-變動通知 → 4-待審核 | `parseInboxNewPromos` | `2-變動通知`（狀態＝`待解析` 的列） | `4-待審核（新戶活動）`；回寫狀態＝`已解析`／`已解析-無新戶活動` | `benefits-parser.gs` |
| 解析新戶活動：3-貼上原文 → 4-待審核 | `parsePastedText` | `3-貼上原文（新戶活動）` A2 原文／B2 卡片提示／C2 網址（**只讀第 2 列**） | `4-待審核（新戶活動）` | `benefits-parser.gs` |
...
- 通知信分兩區：🔴 實質回饋變動（優先看）／⚪ 其餘（通常可略）；標題直接寫「N 筆實質變動 / 共 M 筆」
...
- **一次性**：欄位變了，**請把舊的「2-變動通知」分頁刪掉**讓它自動重建新表頭（那是 log，可拋棄）
...
| 讀取來源 | ① `2-變動通知` 中狀態=`待解析` 的列（監控的產出）② `3-貼上原文（新戶活動）` 分頁手動貼文字 |
...
- 這個分頁是可拋棄的 log：欄位改版時整個分頁刪掉讓它自動重建即可

### 發布變動紀錄 → 詳情頁「近期異動」（2026-07-31 新增）

`2-變動通知` 表尾多了三欄，讓「監控抓到的變動」能一鍵變成卡片詳情頁的「近期異動」列。

✅ **舊分頁不用刪，裡面的內容也不會動到。** 三欄是加在最右邊（「狀態」欄右邊），
`ensureInboxPublishColumns_` 會在下次監控寫入時、或你按「發布變動紀錄」時自動把表頭補上。
補表頭前就存在的舊列，那兩格本來是空的，但**每次按「發布變動紀錄」都會自動回填**
...
⚠️ 回填進去的摘要是 **AI 的內部視角**，跟新列的預填一樣要改寫成給用戶看的話再打 `V`。
萬一自動補失敗（表頭跟程式預期對不上，或那三欄被別的東西佔用了），視窗會告訴你怎麼手動補：
**在第一列「狀態」右邊三格依序填「公開摘要」「公開卡片」「公開」**——順序不能顛倒，
因為監控寫入是照位置填的（`appendRow`），發布則是照欄名找欄。
...
| `公開` | **你打 `V`** | 打了才會發布（`V`／`v`／全形／勾號／核取方塊都吃）。發布後程式改成 `已發布`，重按選單不會重複發布 |

操作只有三步：**改字 → 打 V → 選單「🤖 權益自動化 → 發布變動紀錄（詳情頁近期異動）」**（函數 `publishChangelog`，在 `benefits-parser.gs`）。程式會：
...
4. 寫入成功後把 `公開` 改成 `已發布`
5. 結果直接跳視窗（「發布 N 筆／跳過 M 列」），**不寫進任何分頁**
