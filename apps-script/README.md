...
- **`未覆蓋商家追蹤` 是人工維護的表，⚠️ 程式只准 upsert、絕對不准重建**（2026-09-08 新增，
  ...
  **滾動 30 天覆寫**——它回答得了「現在誰在問我們答不出來的東西」，回答不了「這個詞什麼時候
  ...
  - **自動結案**：本次視窗 `沒對到 = 0` 且 `有對到 > 0`，且該列狀態仍是「未覆蓋」、
    `覆蓋日期` 空白 → 改成「已覆蓋」並寫上覆蓋日期，同時把 `最近視窗未對到次數` 歸 0
    （剛結案卻留著舊次數會自相矛盾）。`最近出現日` **刻意停在最後一次落空那天**，
    跟 `覆蓋日期` 並排才讀得出「從最後一次落空到補好隔了多久」。
    - 只有 `有對到 > 0` 才算數：全是 has_match 註冊前的「未知」資料不能當成補好了。
    - **已覆蓋的詞再度落空時程式不會自動翻回「未覆蓋」**（狀態是人工欄），但
  ...
  - 每次執行的「新增 N 個／自動結案 M 個」寫進「更新紀錄」（`writeLastUpdated()` 第 5 個參數）
    ——新詞要有人去看、自動結案的列要有人回頭確認，不寫在那裡就沒有人會知道表變了。
