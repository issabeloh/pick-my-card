...
- 2026-09-23 新增「站長推薦／行李箱專區」：New Cardholder Promos 多讀 `min_spend`、`luggage_*`、
  `pick_*` 七欄（說明見 `docs/project/data-pipeline.md` 第 9b 節）。同專案另加
...
> ⚠️ **貼進 Sheets 前先確認拿對版本**（2026-09-03 實際踩到：從 `main` 取檔，貼下去等於
> 原地踏步）。**不要用行數判斷**——行數每次改動都會變。改用「搜關鍵字」：這份備份最新
> 加入的函數名（目前是 `updateUncoveredMerchants`，2026-09-08 加入）在舊版裡搜不到，
> 搜得到才是對的檔。
> 取檔一律從該次工作的分支，不是 repo 預設分支。
...
- **主流程**：`updateAllReports()`——每天**下午 5 點**（`atHour(17)`，實際落在 17:00–18:00 之間）
  由時間驅動觸發器叫醒（`createDailyTrigger`，它會先刪掉同名舊觸發器再建，重跑不會長出兩個）。
  ⚠️ 2026-09-14 更正：這裡原本寫「每天 6 點」，與程式不符（`atHour(17)`）——**以程式為準**。
...
- **GA4 按鈕點擊逐日（2026-09-03 新增）**：`GA4_按鈕點擊`（`importGA4ButtonClicksDaily`）
  ...
  回填起點 `BUTTON_CLICK_START_DATE = 2026-06-07`（＝`button_type` 的註冊日）。
  ...
  - ⚠️ **回填起點 2026-06-07 是實測修正的**（2026-09-03）。原本設 `2026-01-01`，假設
    「註冊前那段會回 `(not set)`，至少留得住按鈕點擊總量的逐日趨勢」——**這個假設是錯的**。
    實跑後表上第一列就是 `2026/06/07`，01-01～06-06 **一列都沒有**：查詢帶了
    `customEvent:button_type` 這個維度時，GA4 對註冊日之前的事件是**不回傳**，
    不是回 `(not set)`。往前多要 158 天純粹白跑，只是把一支本來就最重的查詢
    （3 維度 × 數個月）再拉長 2.8 倍——那也是它在 `updateAllReports()` 裡吃到 502 的原因之一。
    要看「不分版位的按鈕點擊總量」逐日趨勢，該查的是**不帶** `button_type` 維度的
    `eventName=button_click`，不是這張表。
  - **502 重試**（`runReportWithOneRetry_`）：GA4 Data API 偶發 502，這支最重的查詢
    2026-09-03 首跑 `updateAllReports()` 就中了一次、單獨重跑立刻成功＝典型暫時性錯誤。
    包一次重試（間隔 3 秒）純粹是**為了不要在「更新紀錄」留一行沒必要的 ⚠️**，
    讓真正該看的失敗不被雜訊淹掉——就算重試也失敗一樣不掉資料（`runStep_` 會隔離，
    表維持上次成功內容，而這張表每天重抓全區間、隔天自己補回來）。
    **只包這一支**：其餘 7 支 GA4 查詢都輕得多、也有數月乾淨紀錄，沒理由一起改。
...
- **`GA4_每月新舊用戶` 是人工維護的表，⚠️ 程式只准 append、絕對不准覆寫**（2026-09-03 訂）：
  ...
  - `總用戶`(B)：**不等於** C+E（GA4 各維度值各自去重，同一人可能當月既算新用戶又出現在
    回訪，加起來會灌水）。2026/08 以前由站長手動補齊，之後由 `appendMonthlyUserRow()` 帶入。
  - **當月那列一律等月結後才建**（2026-09-03 訂）。這樣 I／J 的分子分母永遠同步，
    表上不會出現「還在長的半個月」，`備註` 也才寫得完整。
  - **`appendMonthlyUserRow()`：每月自動補上個月那一列**（2026-09-03 加入，掛在
    `updateAllReports()`）。全檔**唯一會寫進人工維護表的函數**，五個安全性質缺一不可：
    ① **只 append、永遠不 clear**（禁止在這裡用 `writeSnapshotSheet_`——初版就是這樣寫的，
    會把備註每天清空一次）；② **欄位用表頭文字定位、不寫死欄號**（`MONTHLY_COLS`），
    對不到或**欄名重複**就 throw，訊息直接寫進「更新紀錄」（`indexOf` 遇重複只抓最前面那個，
    `cards-export` 就是這樣掉過整組資料）；③ **先把上一列整列 `copyTo` 下來**——公式、
    數字格式、條件式格式一起帶下來，所以佔比／較前月／日均／當月申辦點擊數這些**公式欄
    自動延續，程式裡不重寫任何一條試算表公式**；④ **同月已存在就略過**（它每天跟著跑，
    沒有防重複會每天多一列）；⑤ **只補已完整結束的上一個月**，且要等到次月第
    `MONTHLY_ROW_APPEND_DAY`（預設 **2**）天——刻意不是 1 號，GA4 對最近 1–2 天的數字
    還會事後微調（同批改動實測 `GA4_按鈕點擊` 兩次執行間 155→158 列就是這回事）。
    - **API 成本一個月只花一次**：所有不花 API 的檢查都排在查詢之前，非補列日直接 return，
      平常日這一步的 `runReport` 是 **0** 次；補列當天 +2 次（新舊拆分／整月總數各一支）。
    - **補完的列備註是空的**，`writeLastUpdated()` 會把「已補 YYYY/MM 一列……⚠️ 備註欄待
  ...
  - **`申辦按鈕點擊數`**（K 欄，`備註` 左邊；2026-09-03 由「當月申辦按鈕點擊數」改名——
    **改欄名不影響程式**，`appendMonthlyUserRow()` 只用表頭文字定位它自己要寫的 5 個純資料欄，
    ...
    - ⚠️ **2026/07 以前一律顯示 `—`，這是刻意的**：`GA4_按鈕點擊` 的資料就是從
      2026-06-07（`button_type` 註冊日）才開始存在，更早的月份查得到的是 **0**，
      而 2026/06 只有 24 天。沒有這道閘門，表上會出現「申辦點擊從 0 暴增」的假訊號
      ——那是追蹤上線，不是成效。**2026/07 才是第一個可用的完整月。**
    - **貼上順序不能反**（2026-09-03 實際踩到）：一定是「先貼程式 → 跑
      `importGA4ButtonClicksDaily()` 讓分頁長出來 → 再貼 K 欄公式」。反過來的話
      2026/07 之後那幾列會是 `#N/A`（那張表還不存在），而 2026/07 以前的 `—`
      **照樣正常顯示**——因為 `IF` 先短路了、根本沒碰到那張表。
      所以「一片 `—` 夾幾個 `#N/A`」不是公式壞掉，是分頁還沒生出來。
      分頁補上後 K 欄會自己變成數字，不用重貼公式。
    - **驗算方式**：`=COUNTA('GA4_按鈕點擊'!A4:A)` 貼在任一空格。報錯＝分頁名稱對不上；
      回 0＝分頁在但沒資料（程式沒跑成，看「更新紀錄」）；回約 1,500＝資料正常。

- **`未覆蓋商家追蹤` 是人工維護的表，⚠️ 程式只准 upsert、絕對不准重建**（2026-09-08 新增，
  `updateUncoveredMerchants()`，掛在 `updateAllReports()`）：補的盲區是 `GA4_搜尋落空` 只有
  **滾動 30 天覆寫**——它回答得了「現在誰在問我們答不出來的東西」，回答不了「這個詞什麼時候
  第一次出現」「我們什麼時候補好的」「補完之後真的沒人再問了嗎」。那三題要**跨執行的持久
  記錄**才答得出來。
  - 欄位（站長手建）：`商家詞`／`首次出現日`／`最近出現日`／`最近視窗未對到次數`／
    `歷史最高未對到次數`／`狀態`／`覆蓋日期`／`備註`。表頭上方第 1–2 列由程式寫入標註
    （規則全寫在那兩列，讀表的人不看程式也解讀得了數字）；表頭原本在第 1 列的話，
    **第一次執行會 `insertRowsBefore` 插兩列**把它推到第 3 列——插列不覆寫，既有資料與格式
    一起往下移，之後每次執行都是 no-op。
  - ⚠️⚠️ **未對到次數是「覆寫」不是「累加」**——這是全段最容易寫錯、錯了也不會有人發現的
    一件事。來源視窗滾動 30 天，今天與昨天有 29 天重疊，同一批試算會被連續約 30 次執行各看到
    一次；任何 `舊值 + 本次值` 的寫法都會把同一批事件乘上約 30 倍，數字看起來很嚴重但完全是
    假的，而且因為它「只是偏大」不會 throw。`最近視窗未對到次數` ＝本次執行看到的值；
    歷史嚴重度看 `歷史最高未對到次數`（**取 max，同樣不是加總**）。
  - **upsert 規則**：`沒對到 > 0` 的詞，表上沒有就 append（首次＝最近＝今天、兩個次數欄都填
    本次值、狀態「未覆蓋」）；已經有就只更新 `最近出現日`／`最近視窗未對到次數`／
    `歷史最高未對到次數`。**沒出現在本次視窗的詞整列不動**（數字停在最後一次出現時，
    新鮮度看 `最近出現日`），全對到的新詞**不會**被新增進來。
  - **自動標記「待確認」（⚠️ 不是「已覆蓋」）**：本次視窗 `沒對到 = 0` 且 `有對到 > 0`，
    且該列狀態仍是「未覆蓋」、`覆蓋日期` 空白 → 改成「**待確認**」並寫上偵測到的日期，
    同時把 `最近視窗未對到次數` 歸 0（剛標記卻留著舊次數會自相矛盾）。
    `最近出現日` **刻意停在最後一次落空那天**，跟 `覆蓋日期` 並排才讀得出隔了多久。
    - **分工是「程式只寫觀察、人寫結論」**：程式驗證得了的事實只有「最近 30 天沒人以會落空的
      方式查到它」，那**不等於**我們補好了。`has_match` 是「`findMatchingItem(輸入字串)` 有沒有
      回傳東西」（`js/cashback-engine.js`），而它吃前端**「精準搜尋」勾選框**
      （`js/home-ui.js` 的 `exactOnly`）——同一個字串，勾了要完全一致才算對到、沒勾走放寬比對。
      所以「這 30 天沒落空」有三種可能：① 真的補了資料 ②【誤判】這陣子剛好只有沒勾精準搜尋的人
      查它（「全聯福利中心」勾了會落空、沒勾對到「全聯」）③【誤判】舊的落空事件滾出 30 天視窗了。
    - **「已覆蓋」只有人寫得起，程式只會讀不會寫**。查證資料真的補上後自己把狀態改成「已覆蓋」，
      程式只動狀態為「未覆蓋」的列，改完就永遠不會再被碰。確認是誤判要重新打開的話：
      狀態改回「未覆蓋」**並清掉覆蓋日期**，兩件都做才會重新納入自動判斷。
    - 只有 `有對到 > 0` 才算數：全是 has_match 註冊前的「未知」資料不能當成補好了。
    - **已標記／已覆蓋的詞再度落空時程式不會自動翻回「未覆蓋」**（狀態是人工欄），但
      `最近視窗未對到次數` 會 > 0，用它就篩得出退步的項目。
  - **人工欄保護**：`備註` 程式**永遠不寫**；站長把 `狀態` 設成「不處理」的列，
    `狀態`／`覆蓋日期`／`備註` 從此不再被程式碰，只更新日期與次數欄。
  - 其餘防呆同 `GA4_每月新舊用戶`：欄位用表頭文字定位（`UNCOVERED_COLS`）、**欄名重複就
    throw**、分頁不存在就略過。來源欄位也用表頭文字對位（`UNCOVERED_SOURCE_COLS`），
    `updateGA4MerchantSearches()` 日後加欄不會靜默錯位。
  - **API 成本：每天 1 次 `runReport`**。搜尋資料直接用 `collected.searches`（`GA4_熱門搜尋`
    那步已經抓回來的，不重複打）；額外那一次是 `uncoveredLastMissDates_()` 查落空日期用的。
    `GA4_熱門搜尋` 那步失敗時（`collected.searches === null`）**什麼都不動**、也不會寫一份空資料進去。
  - **`最近出現日` ＝ GA4 事件日期，不是程式執行日**（2026-09-11 修正）：舊版只要視窗裡還留著
    一筆舊落空就把它刷成今天，看起來像「今天還有人落空」，站長因此誤判「昨天修好的 bug 今天
    又復發」。現在由 `uncoveredLastMissDates_()`（維度 `customEvent:merchant` × `date`，
    伺服器端過濾 `eventName=calculate_cashback AND has_match=false`）取每個詞的最大日期。
    - **查不到就整欄不動**（維持上次的正確值），**不退回寫執行日**——那正是被修掉的誤導行為。
    - **`首次出現日` 維持原語義**＝這個詞第一次被本表記錄下來的**執行日**，刻意不改。
    - 刻意不在 `updateGA4MerchantSearches()` 加 `date` 維度：那支的回傳同時餵給 `GA4_熱門搜尋`、
      `GA4_搜尋落空` 與週歷史快照三處，加維度會讓列數乘上天數、撞上 `GA4_SEARCH_ROW_LIMIT`
      把熱門搜尋表截斷。
  - 空框送出（GA4 的 `(not set)`）不列入——那是沒輸入，不是覆蓋缺口。
  - **「未覆蓋」＝全站所有卡片都沒有這個項目，與用戶選了哪幾張卡無關**（站長 2026-09-14 問過）：
    `findMatchingItem()` 掃的是整個 `cardsData.cards`（`js/search-match.js:377`），比對範圍是每張卡的
    `cashbackRates.items` ＋ `specialItems` ＋ `generalItems`（連隱藏 rate 也算），
    只要**任何一張卡**對得上 `has_match` 就是 true。所以這張表上的詞是真正的資料缺口，
    不會因為用戶剛好沒勾到那張卡而被誤記。
  - **剛標成已覆蓋的詞，「最近視窗未對到次數」不會馬上歸零**：來源是滾動 30 天，補好之前那些
    落空紀錄還會留在視窗裡最多 30 天。判斷方式看 `最近出現日` 有沒有**晚於**`覆蓋日期`
    ——晚於＝補了還在落空，要回頭查；早於＝補好後沒再落空過，等數字自己滾掉即可。
  - ⚠️ **商家詞欄 append 前一定要先 `setNumberFormat('@')`**（2026-09-09 首次上線實測到的坑）：
    `setValues()` 寫字串等同「在儲存格打字」，Sheets 照樣做自動判讀——`7-11` 會變成日期
    2026/7/11、`711` 會變成數字。**變成日期就完了**：下次執行讀回來是 `Date`、跟 GA4 的字串
    對不上，那個詞會**每天被重複 append 一次**，原本那列還永遠不會更新。
    （數字沒事，`String(711)` 還是 `'711'` 對得回去。）
    - 分頁上**一眼看出中招**：商家詞靠左＝文字（正常），靠右＝被轉型了。
    - 已經被寫壞的列救不回來（`7-11` 與 `7/11` 會變成同一個日期，反推是猜的），
      程式會在「更新紀錄」點出有幾列中招，**手動刪掉那幾列**，下次執行會以純文字重建。
  - **手動跑一次用 `runUncoveredMerchantsNow()`**（編輯器上方函數選它 → Run）。
    ⚠️ **不要直接選 `updateUncoveredMerchants` 去跑**：它需要參數，而編輯器的 Run 按鈕
    傳不了參數，會拿到 `undefined` 只回一句「略過」——看起來像壞掉，其實只是沒資料。
    手動入口自己抓一次 GA4（1 次 `runReport`），會順便重寫 `GA4_熱門搜尋`／`GA4_搜尋落空`
    （本來就每天覆寫，無副作用），不跑 Clarity、不動其他分頁。
  - 每次執行的「新增 N 個／標記待確認 M 個」寫進「更新紀錄」（`writeLastUpdated()` 第 5 個參數）
    ——新詞要有人去看、標成「待確認」的列要有人回頭查證，不寫在那裡就沒有人會知道表變了。
...
| GA4 Data API（標準資源） | 每資源每日約 20 萬 tokens／每小時約 4 萬；並行 10 個請求；**API 本身不收費，超額是被拒絕不是被計費** | `updateAllReports()` 每天呼叫 `runReport` **8 次**（2026-09-03 新增 GA4_按鈕點擊 前是 7 次；每月補列那天 +2 次＝10 次，其餘日子 `appendMonthlyUserRow()` 在打 API 前就 return），距離配額 3 個數量級以上。`GA4_按鈕點擊` 是**累積期間、每天重抓全區間**，token 成本會隨資料期間緩慢變大——但成長的是單次查詢的列數（每年約 +3,000 列），不是呼叫次數 |
...
| 機制 | 說明 |
|---|---|
| 列數節流（主動） | `MONITOR_CONFIG.maxRowsPerRun`：單次最多實際抓幾列，0 = 不限。**達到上限不會自動接力**，剩下的留給下一次排程觸發器——這是「一週分兩天跑」用的那顆開關，見下方食譜 |
| 看錶煞車（被動） | 每一列**開跑前**先看錶，已超過 `MONITOR_CONFIG.maxRunSeconds`（210 秒）就不再開新的一列。210 的算法：單列最壞 ≈ 抓取 60 秒＋Gemini 40 秒＋重試 sleep 9 秒 ≈ 110 秒，210＋110＝320 秒 < 360 秒 |
| 自動接續 | 被**看錶煞車**停下時排一個 `MONITOR_CONFIG.resumeAfterMinutes`（2）分鐘後的**一次性**觸發器接著跑，不用人工重按。設成 0 就關掉，改回人工重跑。⚠️ 只有看錶煞車會接力：它代表「差點撞牆」，不接的話清單後半段要等整整一週；列數節流是站長刻意要分次跑，接力反而違背本意 |
| 進度游標 | 存在指令碼屬性 `WATCHLIST_RESUME_ROW`（＋時間戳 `WATCHLIST_RESUME_AT`），**逐列更新**——萬一某列離譜地慢、把 6 分鐘硬撞爆，收尾一行都不會跑，那時就只剩這個游標知道跑到哪裡 |
| 游標過期 | 看兩種停法分開算：看錶煞車 60 分鐘（分鐘級接力，窗要收緊——每週排程觸發器若撿到上一輪的舊游標，清單前半段會被**整段靜悄悄跳過**，那比超時失敗更糟）；列數節流 10 天（這個游標本來就要跨天等下次觸發器，用 60 分鐘會讓「分兩天跑」失效成每次都只跑前半段） |
| 段數上限 | **連續**被看錶煞車最多 `MONITOR_CONFIG.maxResumeChunks`（8）段。防「某一列每次都把執行撞爆 → 游標卡死 → 無限接力」把每日 90 分鐘的觸發器額度燒光；達上限會停手並寄信說明。列數節流的段**不算進去**（那是照設定正常收工） |
...
讓它們不必每輪都抓；或把不再需要的列 `active` 改 `FALSE`；或照下面的食譜改成分次跑。

#### 食譜：一週分兩天跑（不用一次跑完）

1. `MONITOR_CONFIG.maxRowsPerRun` 填**大約清單的一半**（清單有幾列 → 選單「體檢填法：1-監控清單」
   會告訴你，或直接看分頁列數）。抓不準就往小的填，反正沒跑完的會自己接下去。
2. 觸發器 → 編輯 → Week timer 改成**每週兩次**（例如週一、週四各一次）。
3. 沒了。第一天跑前半段、停下、進度記在指令碼屬性；第二天的觸發器自動從斷點接著跑完後半段，
   跑完狀態自動清空、下週一又從頭開始。中途要人工插隊也行——手動 Run 一次就是從斷點接著跑。

**好處不只是避開超時**：走 Jina 的頁一次約 5,000 tokens，分次跑等於把 Jina／Gemini 的用量
攤平在兩天，比較不會撞到「每分鐘額度」而出現一整批 `Jina HTTP 429`。

**要怎麼確認它真的在分段**：Apps Script 左側「執行項目」看每次執行的**時長**——
應該都遠低於 360 秒且狀態是「已完成」，不再出現 `Timed Out`。

#### 改這段程式之後要跑的驗證

```bash
node tools/test-watchlist-chunking.js   # 零依賴，不用 npm install
```

把 Apps Script 的服務全部換成假的、整份 eval 進來實跑 8 種情境（不分段基準／看錶接力／
列數節流跨三次排程／游標過期／兩道煞車誰先到／段數上限／每週觸發器沒被誤刪）。
守的三條不變式：**每列剛好抓一次**（重抓＝白燒 Jina 額度）、**一輪跑完屬性全清乾淨**、
**每週觸發器任何情況都不能被刪掉**。有紅字就 exit 1。
...
### ② 到官網現抓超連結（`fillRegisterLinksFromSnapshots`，呼叫 AI）

只處理①標黃的槽位（要登錄、但沒連結）。①沒標黃的槽位完全不會進 AI，省下大半的呼叫。

#### ⚠️ 為什麼不能只靠 `last_snapshot`（2026-09-08 首次實跑 3 張卡、0 個結果的原因）

**監控存的快照是純文字，超連結的網址在存進去之前就被丟掉了**：

- `watchlist-monitor.gs` 的 `fetchDirect_()` 有一行 `.replace(/<[^>]+>/g, ' ')`，把整個 `<a href="...">` 標籤剝掉，只留下錨點文字。官網寫「立即登錄」四個字掛超連結時，快照裡就只有「立即登錄」四個字
- `fetchViaJina_()` 明確送 `X-Return-Format: text`（註解寫「不要 markdown 連結雜訊」）。markdown 格式本來會保留 `[文字](網址)`，text 格式一樣只剩文字

所以快照裡**只可能有「官網把網址當成可見文字印出來」的那種網址**（少數條款頁會這樣寫，兆豐 BT21 那筆就是），絕大多數銀行的「點這裡登錄」通通抓不到。這不是 AI 不夠聰明，是資料源裡根本沒有那個網址。

**解法刻意不是去改監控**：改 `fetchDirect_` / `fetchViaJina_` 會讓每一頁的 `last_snapshot` 內容整批變樣，下一輪監控會把全部頁面都判成「大量變動」，等於製造一次全站假警報。改成這一支自己去官網**現抓一次原始 HTML**、只解析 `<a href>`（`regLinkHarvestLinks_`），完全不碰快照。

#### 流程

1. `regLinkHarvestLinks_()` 對這張卡在 `1-監控清單` 裡的每個 `url` 發一次 GET，用正則抓 `<a href>`；相對網址補成絕對網址（`regLinkResolveUrl_`）；**只留「錨點文字或前方 80 字含『登錄』」的**，最多 `maxCandidateLinks`（40）個
2. 把「候選連結清單（網址／連結文字／前方文字）」＋「①算出的待找槽位」＋「快照文字（只當上下文）」一起送給 Gemini
3. AI 的任務**不是找網址，是把清單裡的網址對應到槽位**——`register_link` 只能原封不動複製清單裡的某一個
4. 硬性檢查：回傳的網址不在候選清單裡 → 直接丟棄並在說明欄記 ⚠️

- 抓不到任何候選連結（JS 動態產生的按鈕、或該頁本來就沒有登錄入口）→ 狀態欄註明，那幾格就是人工去官網補
- **兩道煞車，先到哪個算哪個**：`maxCardsPerRun`（張數上限，2026-09-09 由 3 調成 **6**）與 `maxRunSeconds`（**240 秒**）。真正的煞車是後者——每張卡開跑前先看錶，超過就不再開新的一張
  - ⚠️ 為什麼要看錶：Apps Script 單次執行硬上限 6 分鐘，**超時是直接砍掉**，那一輪最後的 `setBackgrounds` 批次寫入與結果視窗都不會執行（已寫好的連結與狀態欄不受影響，那些是逐張即時寫的）。與其算「幾張剛好不會爆」，不如讓它自己收工
  - 因為時間煞車在，`maxCardsPerRun` 可以放心調大；調大只是讓「順的時候一次多跑幾張」
- `AI 搜尋狀態` 欄有值的卡自動跳過，再按一次選單接著跑；**結果視窗會列出這一輪處理了哪幾張卡與各自的命中數**（時間到而收工時也會講），方便逐張複核
- 快照仍然有用：它提供活動的敘述文字，讓 AI 判斷某個登錄連結屬於哪一個槽位

### ③ 把複核過的連結寫回正式 Cards Data（`applyRegisterLinksToCardsData`，2026-09-09 新增）

站長回報「自己 copy-paste 找格子很花時間、也可能出錯」。改成：在草稿的 **`貼回正式表`** 欄對複核完的那一列打 **V**（`V`／`v`／`✓`／`✔`／`TRUE`／`1` 都收），按選單 ③，程式把那一列的所有 `registerLink_N` 寫進正式表，成功後把 V 換成「已貼上 <時間>」。

**欄位位置的疑慮不存在**：三個工作欄（`貼回正式表`／`登錄連結說明`／`AI 搜尋狀態`）全部加在**最右邊**，Cards Data 原本的欄位一格都沒位移；而且程式是**靠欄名對格、靠 id 對列**，草稿與正式表的欄序、列序完全不同也不會貼錯。

**⚠️ 這是整個自動化檔唯一會寫入正式 Cards Data 的動作。** 七道防線寫死在程式裡：

1. **只寫 `registerLink_N` 欄** —— 每格寫入前過 `regLinkAssertRegisterCol_()`（欄名須符合 `/^registerLink_\d+$/`），沒有任何路徑能碰到 `rate_N`／`items_N`／`conditions_N` 或其他任何一欄
2. **靠欄名對格**，不靠位置
3. **靠 `id` 對列**，不靠列號
4. **不覆蓋既有的不同值** —— 正式表那格已有值且與草稿不同 → 跳過該格並列進「衝突」報告。**站長手改的正確網址因此不會被蓋掉**
5. **一張卡是一個整體** —— 任一格出問題（例如正式表缺該欄）就整張卡不寫，不會寫一半
6. **寫之前跳確認視窗**，列出要寫幾張卡、幾個連結、哪幾張
7. **寫成功才蓋章** —— 沒蓋到就代表沒寫成功，下次會重試；已蓋章的列不重複寫

**驗證方式**：這支在 Sheets 外沒辦法實跑，所以用假的 `SpreadsheetApp` 做了單元測試（草稿與正式表刻意用不同的欄序與列序），確認：靠 id 寫到正確的列、靠欄名寫到正確的格、值相同不重寫、衝突只跳該格而不影響同卡其他格、正式表沒有的 id 整張跳過並回報、沒打勾不寫、已蓋章不重寫、按取消時零寫入、欄名鎖擋得住 `rate_1` 這種非法目標。

### ④ 檢查登錄連結是否有死網址（`checkRegisterLinksAlive`，2026-09-10 新增）

**只讀不寫。** 讀正式 Cards Data 的所有 `registerLink_N`，去重後對每個網址發一次請求，回報死掉的。不碰任何一格資料。

**為什麼是「查死連結」而不是「用 AI 重抓比對現有值」**（2026-09-10 站長與我一起否決了後者）：

站長複核時修正過幾筆 AI 抓錯的連結，而那些正確網址是**人判斷出來的**——AI 重跑一次還是會找到同一個錯的。所以「重抓來比對」每次都會對著站長最用心修正過的那幾張卡誤報，**噪音剛好集中在最不該吵的地方**，只會增加審核工作。

真正的風險是「銀行換網址、舊的死掉」，那個不需要 AI：發個請求看回什麼碼就知道。站長手改的正確連結會回 200、完全不會被提到 → **平常零審核工作，只在真的壞掉時出聲**。

**分兩級回報，刻意寧可漏報也不假警報**：

- **❌ 確定死了**：HTTP 404／410 → 這些要去官網找新網址替換
- **❔ 無法確認**：其餘非 2xx/3xx（銀行網站擋機器人常回 403）→ 只列出來，不當成問題

先送 HEAD 省流量，遇到 405／501／403 再用 GET 確認一次（不少站不接受 HEAD）。同一個網址被多個槽位共用時只發一次請求。有 `maxRunSeconds` 時間煞車，跑不完再按一次接著跑。

**什麼時候跑**：想到就跑，沒有固定頻率。全部正常時它只會說「沒有任何確定死掉的連結」，不佔你任何時間。

### 完整作業流程（SOP，2026-09-09 定案）

每次要補登錄連結時照這個跑，不用回頭翻對話：

| # | 動作 | 在哪裡 | 會發生什麼 |
|---|---|---|---|
| 1 | 選單 **① 標出需登錄的活動（不用 AI）** | 自動化檔 | 首次執行會問你要不要建草稿分頁 → 確定。幾秒跑完全部卡片，不花任何 AI 額度 |
| 2 | 到資料檔看 **`Cards Data-登錄連結草稿`** | 資料檔 | 🟡＝要登錄但沒連結、🟢＝已有連結。右邊 `登錄連結說明` 欄逐槽位列出回饋率／上限／適用通路 |
| 3 | 選單 **② 找登錄連結** | 自動化檔 | 一次最多 6 張卡或 240 秒（先到哪個算哪個）。重複按到出現「需要搜尋的卡片都跑完了」 |
| 4 | **複核** `登錄連結說明` 欄 | 資料檔 | ⚠️ **AI 會抓錯，這一步不能跳過**（2026-09-09 首批實測：確實有幾筆配錯，站長手動改成正確網址）。每一行都附了原文佐證，看它憑什麼把那個連結配到那個槽位；抓錯就直接在該格 `registerLink_N` 改掉。一次配了很多個連結的那張卡最值得看。判準是「這個入口點進去，使用者找不找得到這檔活動的登錄按鈕」 |
| 5 | 在 **`貼回正式表`** 欄對複核完的列打 **V** | 資料檔草稿 | `V`／`v`／`✓`／`✔`／`TRUE`／`1` 都收。**你手改過的網址會照你改的寫回去**——①②③ 三支都不會覆蓋既有的值（①②只填空格、③ 遇到不同值會跳過並回報衝突） |
...
**先講結論：①②③ 不是一次性的補件，是常態流程。** 因為既有的兩支解析器實務上抽不到登錄連結：

| 解析器 | 輸入是什麼 | 抽得到登錄連結嗎 |
|---|---|---|
| 解析新卡：3-貼上原文 | 站長從官網**複製貼上的文字** | ❌ 幾乎抽不到——瀏覽器複製只會帶走可見文字，`<a href>` 的網址留在原地 |
| 解析活動更新：2-變動通知 | 監控存的「新文字」（`last_snapshot` 同源） | ❌ 抽不到——`fetchDirect_` 的 `.replace(/<[^>]+>/g, ' ')` 早就把 `<a>` 標籤剝掉了 |
| **② 找登錄連結** | **到官網現抓原始 HTML，解析 `<a href>`** | ✅ 這是唯一撈得到的路徑 |

兩支解析器的 AI schema 確實有 `register_link` 欄位（2026-09-08 加的），偶爾官網把網址當可見文字印在條款裡時會抽到——但那是少數，**不能當成主要來源**。

所以日常是這樣：

- **新增一張卡**：照常走「解析新卡」→ 審核 → 貼進 Cards Data。**記得把該卡的官網頁面加進 `1-監控清單`**（本來為了監控就要加）。然後跑一次 ①② —— ① 會標出這張新卡有哪些槽位提到「登錄」，② 只會問這張卡（其他卡的 `AI 搜尋狀態` 已經有值會自動跳過），很便宜
- **既有卡改動 conditions**：如果改完之後多了「需登錄」的槽位，① 會把它標成 🟡，② 接著補連結。沒有新增登錄槽位的改動則什麼都不會發生
- **官網換了登錄網址**：目前偵測不到（③ 的「不覆蓋既有的不同值」只擋衝突、不會主動發現失效）。要重撈就把該卡的 `AI 搜尋狀態` 欄清空再跑 ②

**增量跑很便宜**：① 是純機械、幾秒跑完全部卡片；② 只處理「① 標黃且還沒有連結」的槽位，已經有連結或已搜尋過的一律跳過。所以平常就是「動過資料之後順手按一下 ①，看有沒有新的黃格子」。

⚠️ **要整批重來**（例如想用最新的官網內容重撈全部）：把草稿分頁刪掉，重按 ①。草稿是 Cards Data 的複本，刪掉不影響任何正式資料。
...
- **AI 只能從「程式剛剛從官網 HTML 抓下來的候選連結清單」裡挑一個**，回了清單以外的網址一律丟棄。硬性機械檢查、不是靠 prompt 拜託——LLM 生一個「看起來很像那家銀行」的網址是這個任務最可能出的錯，錯的登錄連結會把用戶帶到 404，**比沒有連結更糟**。被擋下的會用 ⚠️ 寫進說明欄
- ②會對銀行官網發 GET（每張卡最多幾頁），這是**唯讀**行為、與監控無關，不會寫回 `last_snapshot`
