...
| 4 | **複核** `登錄連結說明` 欄 | 資料檔 | ⚠️ **AI 會抓錯，這一步不能跳過**（2026-09-09 首批實測：確實有幾筆配錯，站長手動改成正確網址）。每一行都附了原文佐證，看它憑什麼把那個連結配到那個槽位；抓錯就直接在該格 `registerLink_N` 改掉。一次配了很多個連結的那張卡最值得看。判準是「這個入口點進去，使用者找不找得到這檔活動的登錄按鈕」，**不是「AI 有沒有抓到最精確的網址」**——理由見下方 |
...
#### 為什麼複核跳不掉：登錄入口本身就是模糊的

站長 2026-09-09 首批複核時修正過幾筆，2026-09-10 釐清了原因——**不是 AI 判斷失誤，是資料源本身沒有答案**：

> 我說的連結入口其實還是統一的入口，可能多個活動一起在同一入口。AI 抓的和我抓的多是這種入口，那是銀行的問題，設立相似的頁面卻沒有說明是針對哪一張卡、哪個活動。

也就是說：銀行常常給一個涵蓋多檔活動的共用登錄頁，頁面上不寫「這是哪張卡、哪個活動」。同一張卡的官網上可能有好幾個這種長得很像的入口，**哪一個最貼近某個槽位並沒有客觀答案**——AI 判斷不了，人也是憑經驗挑一個比較接近的。

**所以不試圖用 prompt 修這件事**（2026-09-10 曾加過一條「不要選總入口頁」的規則，當天就移除——前提是錯的：具體的那個連結多半根本不存在）。

**結論：複核是必要成本，不是可以優化掉的東西。** 複核時的判準也不是「AI 有沒有抓錯」，而是「這個入口點進去，使用者找不找得到這檔活動的登錄按鈕」。找得到就夠好了；你找到更貼近的就直接改那一格（③ 不會覆蓋你手改的值）。
