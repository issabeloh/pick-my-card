...
- **GA4 事件／申辦點擊（2026-07-31 新增）**：補上原本最大的盲區——舊版所有表只回答「有多少人
  ...
  - `GA4_事件成效`（`updateGA4Events`）：維度 `eventName`，含 `calculate_cashback`（核心功能
    ...
    週快照存進 `GA4_事件成效_歷史`（2026-08-03 補，其餘 5 張表 2026-07-31 就有了）——
    核心功能使用量的週趨勢是這張表最主要的用途，滾動 30 天視窗自己看不出成長或衰退。
...
- **歷史快照累積（2026-07-31 新增）**：滾動視窗表每次都覆寫，今天蓋掉昨天 → 「關鍵字排名從
  ...
  `GA4_事件成效_歷史`（2026-08-03 補）／`GA4_申辦點擊_歷史`／`GA4_熱門搜尋_歷史`
  （append，永不覆蓋）。
  ...
  - **同日重複寫入（2026-08-03 修）**：`HISTORY_LAST_SNAPSHOT_DATE` 只擋自動流程，
    `snapshotHistoryNow()` 走 `force=true` 會把兩道全域檢查一起跳過——同一天跑第二次就把
    整份快照原封不動再 append 一遍，**不會報任何錯**。2026-07-31 導入當天為了補欄位改了三版、
    每版都手動種一次資料點，`GA4_頁面成效_歷史` 與 `GA4_流量來源_歷史`（那天表頭沒動、三次都
    寫得進去）因此各多出兩份一模一樣的 07-31 快照；`GA4_申辦點擊_歷史`（加了卡片ID）和
    `GA4_熱門搜尋_歷史`（加了 has_match）反而被表頭檢查擋下來才沒重複。
    現在 `appendHistoryRows_` 多收一個 `snapshotDate`，**表頭檢查通過後**再比對最後一列的
    快照日期，同一天就回 `null` 不寫（更新紀錄寫「今日已有快照略過 …」）。
    - 讀最後一列而非掃全表：快照永遠整批 append 在尾端。A 欄讀回來可能是 Date 物件
      （Sheets 會把 `yyyy-MM-dd` 字串自動轉型），所以 `lastSnapshotDateOf_()` 先轉回字串再比。
    - ⚠️ **既有的重複列要手動刪**：程式只防未來，不會回頭清資料。請把 `GA4_頁面成效_歷史`、
      `GA4_流量來源_歷史`（以及 `GSC_關鍵字_歷史`、`GSC_頁面_歷史`，同樣條件下也可能中招）
      裡 2026-07-31 的重複區塊刪到只剩一份——重複列會讓任何加總／平均的趨勢圖直接翻三倍。
...
| **自動化檔**「PMC 自動化流程」 | `1-監控清單`／`2-變動通知`／`3-貼上原文（新戶活動）`／`待審核-*` | `watchlist-monitor.gs`＋`benefits-parser.gs` |
