...
| 4 | **複核** `登錄連結說明` 欄 | 資料檔 | ⚠️ **AI 會抓錯，這一步不能跳過**（2026-09-09 首批實測：確實有幾筆配錯，站長手動改成正確網址）。每一行都附了原文佐證，看它憑什麼把那個連結配到那個槽位；抓錯就直接在該格 `registerLink_N` 改掉。一次配了很多個連結的那張卡最值得看。**已知的錯誤樣態見下方** |
...
#### 已知的 AI 錯誤樣態：拿「活動總入口頁」充當登錄連結

站長 2026-09-09 首批複核時發現的規律（不是零星失誤，是有樣態的）：

> AI 抓到的網址也是登錄各種信用卡活動的入口，但我在該活動頁面上找到更直接的 CTA 連結，就用該連結取代。

銀行網站常有一個「信用卡活動登錄專區／活動總覽」的頁面，列出該行所有活動、每個活動各自有自己的登錄按鈕。那種頁面對「這一組活動要去哪登錄」來說太籠統——使用者點進去還要自己找，跟沒給差不多。

**已針對這個樣態改 prompt**（`regLinkAskGemini_` 的規則 2a）：連結文字若是「活動登錄」「登錄專區」「活動總覽」這類**沒有指名是哪一檔活動**的字眼，而清單裡另有明確對應到該槽位的連結，一律選具體的那個；如果清單裡**只有**總入口頁，回空陣列、不要退而求其次——人工找得到更直接的入口，給一個籠統的反而會蓋掉正確答案。

⚠️ 改了 prompt 不代表就不會錯，**複核仍然是必要成本**。之後再遇到錯的，把「哪張卡／AI 給的錯網址／正確的是什麼」記下來，累積成第二個樣態再調。
