...
- **`未覆蓋商家追蹤` 是人工維護的表，⚠️ 程式只准 upsert、絕對不准重建**（2026-09-08 新增，
  ...
  **滾動 30 天覆寫**——它回答得了「現在誰在問我們答不出來的東西」，回答不了「這個詞什麼時候
  ...
  - **API 成本 0**：資料直接用 `collected.searches`（`GA4_熱門搜尋` 那步已經抓回來的），
    不另外打 GA4。那一步失敗時（`collected.searches === null`）**什麼都不動**、
    也不會寫一份空資料進去。
...
- 一次 `REGLINK_CONFIG.maxCardsPerRun`（**預設 3**）。`AI 搜尋狀態` 欄有值的卡自動跳過，再按一次選單接著跑；**結果視窗會列出這一輪處理了哪幾張卡與各自的命中數**，方便逐張複核
