...
| 處理變動通知：公開／封存／刪除 | `processInboxRows` | `2-變動通知` 的「公開」欄（照打的字分派三種動作） | 公開→**跨檔**寫資料檔的 `變動紀錄`＋回填「已發布」；封存→`2-封存（變動通知）`；刪除→整列刪掉 | `benefits-parser.gs`（搬/刪的執行在 `watchlist-monitor.gs`） |
| 整理版面：2-變動通知 | `tidyInboxLayout` | `2-變動通知` 第一列表頭 | 只搬欄位順序／欄寬／凍結表頭，**不動任何一格的值** | `watchlist-monitor.gs` |
...
舊分頁要換成這個順序：選單 **「🤖 權益自動化 → 整理版面：2-變動通知」**（`tidyInboxLayout`）。
它只搬欄、順便統一欄寬與凍結表頭，**不動任何一格的值**；你自己加的欄會排在最後面保留下來。
`變動段落／舊文字／新文字` 是留給程式讀的（各上限 8 千／4 萬／4 萬字），推到最後就不用捲過它們了。
...
### 「公開」欄＝一個按鈕跑完一輪（公開／封存／刪除）

一列處理完之後要它消失，不用另外找按鈕：**在「公開」欄打字，按一次
...
**在第一列任何三個空欄的表頭填上「公開摘要」「公開卡片」「公開」**——擺哪、什麼順序都行
...
| `公開` | **你打「公開」** | 打了才會發布（`公開`／`V`／`v`／全形／勾號／核取方塊都吃）。發布後程式改成 `已發布`，重按選單不會重複發布。這一欄也吃「封存」「刪除」，見上面「一個按鈕跑完一輪」 |

操作只有三步：**改字 → 在「公開」欄打「公開」→ 選單「🤖 權益自動化 → 處理變動通知：公開／封存／刪除」**
