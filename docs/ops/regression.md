...
**基準快照**：`20260910-140838`（2026-09-10 重拍；上一版 `20260908-165106` 累積到 6 組紅燈）。
本次 6 組差異已逐條確認：4 組只是 conditions 文案修訂；#5／#12 的回饋率下降
（星展 eco 3.5%→1%、藝FUN 3%→0.5%，上限 NT$12,000／NT$20,000→無上限）源自 `domesticBonusRate`
與 `domesticBonusCap` 在 `20260909-165829`→`20260909-170204` 之間由 2.5 改為 null（國內加碼活動結束），
`buildBasicCashbackResult()` 少掉加碼層即為此數字——屬資料變動，非計算回歸。
