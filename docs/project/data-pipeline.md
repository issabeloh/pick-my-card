...
- **手機專屬的版面調整一律寫進 `@media (max-width: 1024px)`**（2026-09-21 立）：
  ...
  - 目前這段裡有：單檔卡按鈕移到身分列下方（`order: -1`）、附屬列收掉沒有圖片時的
    空位與金額欄寬、禮物名稱不截斷、摘要與附屬列標題多顯示一行
...
- **第 2 檔起是「附屬列」不是卡片**（取代原本的卡疊卡）：一行一檔的三欄清單列
  ...
  - **禮物名稱的規格：20px／`--pmc-hero-green`／不截斷，兩個寬度一致**
    （2026-09-21 站長）。`.promo-act-reward.is-gift`（主活動）與
    `.promo-sub-row.is-gift .promo-sub-title`（附屬列）**必須同一組規格**——
    一大一小、一橘一綠看起來就是壞掉的。配色沿革：`#8a5c00` → `#c2410c` → `#10b981`
  ...
  - **收合那行在手機只留「活動詳情 ▾」**：meta 文字（首刷禮那列就是達成條件）改在
    展開後的最上方出現。生成器把同一段文字輸出兩份（`.promo-sub-metatext` 給桌機、
    `.promo-sub-summary` 給手機），CSS 依寬度只顯示一份——刻意不在 promos.js 搬節點，
    那會讓「收合時長什麼樣」變成要跑過 JS 才知道的事
