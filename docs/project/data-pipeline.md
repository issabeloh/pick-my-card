...
- **第 2 檔起是「附屬列」不是卡片**（取代原本的卡疊卡）：一行一檔的三欄清單列
  ...
  - **金額欄是「內容寬＋下限 62px」不是固定寬**（2026-09-21 站長截圖回報）：原本固定
    86px 是照「NT$12,000」這種五位數的最壞情況抓的（實測 84.13px），但現行資料最寬只有
    NT$300＝60.48px，於是首刷禮小標（49px）右邊白白空出 37px。改成 `min-width` 之後
    三位數金額與首刷禮小標一律撐到 62px（同卡標題欄仍然對齊），四、五位數自己長出去。
    ⚠️ `white-space: nowrap` 不能省，不然它不是長出去而是折行。手機版連這個下限也讓掉
    （`min-width: 0`），那邊沒有對齊需求、空間全給文字
  - **禮物名稱：`--pmc-hero-green`／不截斷，兩個寬度一致；字級主活動 20px、附屬列 17px**
    （2026-09-21 站長）。`.promo-act-reward.is-gift`（主活動）與
    `.promo-sub-row.is-gift .promo-sub-title`（附屬列）**顏色必須一致**——一橘一綠
    看起來就是壞掉的；**字級刻意分兩階**，附屬列的 17px 跟同列金額欄 `.promo-sub-amt`
    同高，維持「主活動大、附屬列小」的層級。配色沿革：`#8a5c00` → `#c2410c` → `#10b981`
  - **「首刷禮」小標的配色必須與 `.promo-type-badge--gift` 同一組 token**（糖果粉）。
    2026-09-21 修掉一個 bug：`.promo-sub-tag` 原本硬寫 `#d1fae5/#065f46`，那是
    **定額回饋**的薄荷綠，於是同一張卡上「首刷禮」出現粉、綠兩種顏色
  - **收合那行一律只留「詳情 ▾」**（2026-09-21 晚上起兩個寬度都是；主活動那顆維持
    全稱「活動詳情 ▾」，附屬列的欄位窄、縮成兩個字把寬度讓給標題）：meta 文字
    改在展開後的最上方出現，生成器只輸出一份，`.promo-sub-metatext` 連同它的
    media query 顯示切換整組刪掉了。刻意不在 promos.js 搬節點——那會讓「收合時長什麼樣」
    變成要跑過 JS 才知道的事
  - **meta 文字的去處看類型**：首刷禮（達成條件）與回饋加碼（「最多可拿…・上限消費…」）
    是完整句子，走 `<dl>` 之前的 `<p class="promo-sub-summary">`；**定額回饋**的 meta 是
    「OPENPOINT」「刷卡金」「即享券」這種單一名詞，單獨一段看不懂在講什麼，改走 `<dl>`
    的第一列並掛上 `<dt>回饋類型</dt>`（站長 2026-09-21）。`pmcRenderPromoDetail_()`
    因此有 `leadHtml`（段落）與 `leadRowHtml`（`<dl>` 第一列）兩個鉤子
  - 獎品圖點了開 lightbox（沿用 `setupGiftLightbox`，`stopPropagation` 擋掉展開詳情）；
    同組只要有一列有圖，沒圖的列補 `.promo-sub-thumb.is-empty` 等寬空位，標題才對齊
  - meta **不帶類型名**：類型在卡片層的篩選 chip 已經有。（「上限消費」曾為了省寬度縮成
    「上限」，meta 移進詳情之後不再需要，已還原成全稱）
  - **收合時一行截斷、展開時整句攤開**（2026-09-21 站長）：`[aria-expanded="true"]` 才把
    `white-space` 放回 `normal`
  - 🔴 **對齊方式不可以綁 `[aria-expanded]`**：非首刷禮列的 `align-items: flex-start`
    （金額欄對齊標題第一行）寫成**無狀態**的常駐規則，首刷禮列則一律置中。
    綁狀態會讓每一列在點開的瞬間整排跳一次位——站長 2026-09-21 回報的
    「promo-sub-tag 和圖片點開會跑位」就是這樣來的，詳見教訓記錄
...
- [2026-09-21] `/promos` 附屬列「點開之後上面的小標和獎品圖跟著跑位」：兇手是把
  ...
  **通則：只有「因為內容變高而不得不動」的東西可以動，對齊方式本身要與展開狀態無關。**
  ...
  驗收方式：Playwright 量同一顆 `.promo-sub-tag` 展開前後的 `x/y`，要**逐字相同**，
  不能只用肉眼看。
