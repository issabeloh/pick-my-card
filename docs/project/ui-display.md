...
- **膠囊本體中性化**：底 `#f9fafb`、框 `#e5e7eb`、卡名近黑 `#111827`。站長要求「pill 上完全不要有藍色」——原本的 `#eff6ff` 淡藍底＋`#1e40af` 藍字全部拿掉，讓左半的銀行色塊成為整顆膠囊**唯一**的顏色
  - 卡名 2026-09-09 由 `#6b7280`（4.63:1，剛好過 AA）**改深**成 `#111827`（16.1:1）：站長回報灰字在 33 顆膠囊裡看不清楚。⚠️ **只准更深、不准調淺**：`#8b929c` 只有 3.00:1、`#9ca3af` 只有 2.43:1，那不是「低調」是「看不到」
- **左半色塊 = 品牌色 50% 疊在膠囊底上**，文字取黑或白（哪個對比度高用哪個）。由 `applyBankChipColor()`（`js/home-ui.js`）即時算並寫成 inline style；`styles.css` 的 `.card-chip-bank` 只留「查不到色碼時」的中性灰 `#eceef1`，刻意不寫任何有色相的值
  - 實測**全部是黑字勝出**。公式仍保留白字分支——日後新增更深的品牌色會自動切換，不用有人回頭檢查
- **銀行色塊與卡名之間有一條 4px 粗豎線**（2026-09-09 新增，站長指定的雙色辨識設計）：色塊＋豎線兩色一起認銀行，比單一底色好分。豎線色與兩家的底色覆寫寫在 `js/home-ui.js` 的 `CARD_CHIP_BANK_ACCENT`
  - 玉山／遠東／滙豐／台新／永豐 `#ffffff`、富邦 `#009e9a`、聯邦 `#004ea1`、國泰 `#fdf500`、一銀 `#b3863b`、星展 `#000000`、中信 `#007166`；**表上沒有的發卡行不畫豎線**（與「查不到色碼就保持中性灰」同一原則：沒顏色比錯顏色好）
  - 底色覆寫只有兩家，都是「Sheets 的 BankColors 那支色不適合當膠囊底」：星展原為純黑，50% 疊出來是一塊死灰 `#7c7d7e`、字讀不清（站長回報）→ 底改 `#ec1d25`、黑色移去當豎線；中信原本的綠 `#007166` 移去當豎線 → 底改 `#e92429`。覆寫值仍走同一條 50% 疊底公式（填的是品牌色、不是最終底色），豎線則用原色不疊
  - ⚠️ 這張表刻意是**前端常數、不是 Sheets 資料**：豎線是版面設計不是發卡行資料，而且這版是站長要求「試看」的 mockup。若日後轉正、要讓非工程的人改色，再比照 BankColors 搬進 Sheets
